...
IMAGES = re.compile(r"\.jpg$|\.cr2$|\.jpeg$|\.tiff$|\.tif$",re.IGNORECASE)
		...
		for hash,files in self.currentBackupSet.items():
			if not self.previousBackupSet.has_key(hash):
				...
				for previousEntry in previousFiles:
					for file in files:
						if path.basename(file[0]) in previousEntry[0]:
							found = True
						else:
							equals.append(previousEntry[0])
				if not found:
					if self.options.verbose: 
						print warn("Duplicate:"), "image already backed up under different name: %s == %s" % (files,equals)
...
def update(opt, value, parser, *args, **kwargs):
	...
	if not data.has_key("version"):
		print >> sys.stderr, "Could not load update information"
		sys.exit(1)
	if data["version"] > VERSION:
		answer = raw_input("New version available %s, do you want to update? [Y/n] " % data["version"])
		if not answer in ("n","N"):
			...
			if not data.has_key(currentmd5):
				print >> sys.stderr, "The current script is not updatable, sorry!"
				sys.exit(1)
			elif data[currentmd5] != VERSION:
				print >> sys.stderr, "The current script is not updatable, sorry!"
				sys.exit(1)
			if md5 != data["md5"]:
				print >> sys.stderr, "Error in update, hash mismatch!"
				sys.exit(1)
