...
VERSION="1.9"
UPDATE_URL = "http://lystrup.langdal.dk/imagebackup/backupdates"
...
FIELDS = {	"1.0" : ["hash","filename","filesize","backupset","stored_to_external","media_title"],
				...
				"1.7" : ["hash","filename","filesize","backupset","stored_to_external","media_title"],
				"1.8" : ["hash","filename","filesize","backupset","stored_to_external","media_title"],
				"1.9" : ["hash","filename","filesize","backupset","stored_to_external","media_title"],
				}

IMAGES = re.compile(r"\.jpg$|\.cr2$|\.jpeg$|\.tiff$|\.tif$|\.avi$|\.mov",re.IGNORECASE)
PROGRESS_PATTERN=re.compile(r"(\d+\.\d{2})%")
RANGE_FILENAME = re.compile(r"(?P<year>\d{4})((?P<month>\d{2})((?P<day>\d{2})((_|-)(?P<hour>\d{2})((?P<minute>\d{2})((?P<second>\d{2}))?)?)?)?)?")
RANGE = re.compile(r"((?P<year>\d{4})(:(?P<month>\d{1,2})(:(?P<day>\d{1,2})(:(?P<hour>\d{1,2})(:(?P<minute>\d{1,2}))?)?)?)?)-((?P<year2>\d{4})(:(?P<month2>\d{1,2})(:(?P<day2>\d{1,2})(:(?P<hour2>\d{1,2})(:(?P<minute2>\d{1,2}))?)?)?)?)")
CONTINUEERRORS = (1,)
...
class Backup:
	...
	def prelaunchCheckThumbnails(self):
		if self.options.thumbnails:
			...
			if cmd == "": 
				self.options.thumbnails = False
				if self.options.verbose: print blue("%s not found, skipping thumbnail creation" % self.thumbnailCommand)
			else: 
				...
				if not path.isdir(self.options.thumbnail_directory):
					if self.options.verbose: print blue("Creating thumbnail directory %s" % self.options.thumbnail_directory)
	...
	def handleMetaData(self,metadata):
		...
		if self.options.verbose: print blue("Handling %s" % (metadata))
			...
			for file in files:
				...
				if not self.options.dry_run:
					...
					if self.options.verify:
						# TODO Create verification of copied files
						pass
					if path.isfile(target):
	...
	def parseLine(self,line):
		"""Parse a single line from backup info file"""
		# TODO Make generic parse of lines in backup info file
		s = line.strip().split(",")
	...
	def createLine(self,hash,target):
		"""Creates a single line for backup info file based upon hash value and filename"""
		stat = os.stat(target)
	...
	def startBurn(self):
		...
		if fileCount == 0:
			...
			print "%d files found, total size %d mb" % (fileCount,(totalSize/1024/1024))
		...
		if self.options.range != "all":
			...
			format = "%Y%m%d"
			default = "%s-%s" % (self._range_min.strftime(format),self._range_max.strftime(format))
		...
		if self.metadata.has_key("medialist"):
			self.metadata["medialist"] = self.mediaTitle + "," + self.metadata["medialist"]
		else:
			self.metadata["medialist"] = self.mediaTitle
		...
		if self.storeFiles(filesToBeStored,previousBackupSet):
			if not self.options.dry_run:
				...
				if self.options.verbose: print blue("Skipping write of backup info file " + path.join(self.options.destination,self.options.backup_info_file))
		else:
	...
	def doStore(self,files,backupSet):
		self.tmpBackupInfoFile = NamedTemporaryFile(mode='wr', bufsize=-1, suffix='', prefix='tmp', dir=None)
		...
		if self.options.iso:
			...
			pathSpecsFile = NamedTemporaryFile(mode='wr', bufsize=-1, suffix='', prefix='tmp', dir=None)
			#pathSpecs = ""
			for source,target in files:
				s = path.join(self.options.destination,source)
				pathSpecsFile.write('%s=%s\n' % (target,s))
			pathSpecsFile.write('%s=%s\n' % (path.basename(sys.argv[0]),path.abspath(sys.argv[0])))
			pathSpecsFile.flush()
			cmd = mkisofsCmd+" -path-list "+pathSpecsFile.name
			if self.options.debug:
				print red("\nRunning %s" % cmd)
				raw_input("Press enter ")
			runCmd(cmd,self.showProgress)
	...
	def showProgress(self,line="0",err=0,errMsg=""):
		if err != 0:
			self.o.end(err,errMsg)
			if not err in CONTINUEERRORS: self.abort(err)
	...
	def storeBackupInfo(self,backupSet,file,close=True,backup=False):
		...
		if self.options.verbose: print blue("Storing backup info file " + file)
		f = open(file,"w")
		f.write(header)
		f.writelines(lines)
		f.flush()
	...
	def includeFile(self,entry):
		...
		if include and self.options.range != "all":
			if not self.__dict__.has_key("_range_groups"):
				m = RANGE.match(self.options.range)
				self._range_groups = m
				year 	= m.group("year")
				month = m.group("month")
				day 	= m.group("day")
				hour	= m.group("hour")
				...
				year 	= year and int(year) or 1900
				month = month and int(month) or 1
				day	= day and int(day) or 1
				hour	= hour and int(hour) or 0
				minute= minute and int(minute) or 0
				self._range_from_date = datetime(year,month,day,hour,minute)

				year 	= int(m.group("year2"))
				month = m.group("month2")
				day 	= m.group("day2")
				hour	= m.group("hour2")
				minute= m.group("minute2")
				year 	= year and int(year) or 9999
				month = month and int(month) or 12
				hour	= hour and int(hour) or 0
				minute= minute and int(minute) or 0
				if day == None:
					day = 31
				...
				self._range_to_date = datetime(year,month,day,hour,minute)

				#print self._range_from_date
				#print self._range_to_date
				self._range_max = self._range_from_date
				self._range_min = self._range_to_date
				
			filename = path.basename(entry[self.backupInfoFields.index("filename")-1])
			m = RANGE_FILENAME.search(filename)
			if m:
				#datetime(  	year, month, day[, hour[, minute[, second[, microsecond[, tzinfo]]]]])
				filedate = datetime(fixNumber(m.group("year"),1900),
											fixNumber(m.group("month"),1),
											fixNumber(m.group("day"),1),
											fixNumber(m.group("hour"),0),
											fixNumber(m.group("minute"),0),
											fixNumber(m.group("second"),0))

				if filedate < self._range_from_date or filedate > self._range_to_date: 
					include = False
				else:
	...
	def progressShower(self,progress):
		"""Callback function used to display progress"""
		self.o.setProgress(int(progress))
		self.o.update()
		
	def createThumbnails(self):
		"""Creates thumbnails for all existing images"""
	...
	def printStatistics(self):
		"""Shows some relevant statistics of the current backup set"""
...
def runCmd(cmd,callback):
	...
	err = child.wait()
	if err != 0: 
		#print >> sys.stderr, bad('%s failed w/ exit code %d' % (cmd, err))
		callback(err=err,errMsg="%s failed!" % cmd)
	return lines,errors
	#print errors
	#print lines
...
def fixNumber(number,default=0):
	if number == None: return default
...
def main():
	...
	adv_group.add_option("--debug",action="store_true",default=False,help="Enable debug mode")
